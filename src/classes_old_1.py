...
class LorenzModel():
    def __init__(self, tau=0.1, int_steps=10, sigma=10.,
                 beta=8 / 3, rho=28., ic=np.array([]), ic_seed=0,standardized=False, mean=0.,std=0.):
        self.D = 3
        self.tau = tau
      ...
      def __init__(self, D=3, tau=0.1, sigma=0.1, theta=0.7, mu=0, ic=np.array([]), ic_seed=0,noise_seed=10,discard_len=1000):
          ...
          if ic.size == 0:
            ...
            self.state = ic.flatten()
                ...
                raise ValueError
      def run(self, T):
            model_output = model_forward_Orstein_Uhlenbeck(self.state,T, discard_len=self.discard_len, h=self.h,params=self.params,rng=self.rng)
            self.state = model_output[-1]
            return model_output
...
class Kursiv():
    ...
    def run_array(self, u0_array):
        if self.standardized:
            model_output = kursiv_run_array(self.unstandardize(u0_array), tau = self.tau, N = self.D, d = self.d, params = self.params,
                           const=self.const)
            return self.standardize(model_output)
        else:
...
class Reservoir():
    def __init__(self, reservoir_size=300, input_size=30, spectral_radius=0.6, avg_degree=3,
                 ...
                 standardized=False, mean=0., std=0., noise_seed = 10, Uhlenbeck_noise=False,Uhlenbeck_process=None):
    ...
    def get_train_mats_out_dual(self, data, T, discard_len,return_error=False):
        features = \
            self.get_features(data, T, discard_len, train=True)
        output = data[discard_len + 1:discard_len + T + 1]
        if discard_len == 0:
            output = data[:T]
        info_mat = (features @ features.T) / T
        if return_error:
            return info_mat, features / np.sqrt(T), output / np.sqrt(T), T, features
        return info_mat, features / np.sqrt(T), output / np.sqrt(T), T

    def get_train_mats_out_random_sample_dual(self, data, T, T_all, discard_len, return_error=False,input_features=False,features_all_input=None,output_all_input=None,seed=1):
        if input_features:
    ...
    def get_train_mats_out(self, data, T, discard_len, return_error=False):
        ...
        target_mat = (features.T @ output) / T
        if return_error:
            return info_mat, target_mat,features
        else:
            return info_mat, target_mat
    def get_train_mats_out_random_sample(self, data, T, T_all, discard_len, return_error=False,input_features=False,features_all_input=None,output_all_input=None,seed=1):
        ...
        if return_error:
            ...
            return info_mat, target_mat
    def get_train_error(self, output, features):
    ...
    def get_train_mats_out_noise_in_target(self, data, T, discard_len, return_error=False):
        ...
        target_mat = (features.T @ output) / T
        if return_error:
            return info_mat, target_mat, features, output
        else:
            return info_mat, target_mat
    def get_train_mats_out_noise_only_in_target(self, data, T, discard_len, return_error=False):
        ...
        if return_error:
            ...
            return info_mat, target_mat
    def get_train_mats_out_noise_in_info_mat(self, data, T, discard_len, return_error=False):
        features = \
            self.get_features(data, T, discard_len, train=False)
        output = data[discard_len + 1:discard_len + T + 1]
        output = output
        if discard_len == 0:
            output = data[:T]
        info_mat = (features.T @ features) / T
        info_mat = info_mat + self.rng.normal(size=info_mat.shape) * self.train_noise
        target_mat = (features.T @ output) / T
        if return_error:
            return info_mat, target_mat, output
        else:
            return info_mat, target_mat
    def get_train_mats_out_noise_in_features(self, data, T, discard_len, return_error=False):
        features = \
            self.get_features(data, T, discard_len, train=False)
        features = features + self.rng.normal(size=features.shape) * self.train_noise
        output = data[discard_len + 1:discard_len + T + 1]
        output = output
        if discard_len == 0:
            output = data[:T]
    ...
    def train_from_mats_dual(self, info_mat, features_n, output_n, T, features=None, return_error=False, output=None):
        self.W = features_n.T @ np.linalg.inv(info_mat + np.diag(
            np.ones(T) * self.regularization)) @ output_n
        if return_error:
            train_preds = features @ self.W
            train_error = np.sqrt(mean_numba_axis1((train_preds - output) ** 2.0))
            return train_preds, train_error
    
    def train_from_mats_sep_reg(self, info_mat, target_mat, return_error=False, features=None, output=None):
        _,eigvec=np.linalg.eigh(info_mat)
        reg_mat=eigvec@np.diag(self.regularization)@eigvec.T
        self.W = np.linalg.solve(info_mat + reg_mat, target_mat)
        print(self.W.shape)
        if return_error:
            train_preds = features @ self.W
            train_error = np.sqrt(mean_numba_axis1((train_preds - output) ** 2.0))
    ...
    def sync_and_predict_open_loop(self, data, T, sync_len=0):
        self.r = np.zeros(self.r.size)
        prediction, self.r, self.feature = sync_and_predict_reservoir_open_loop(data, self.r, sync_len, T, self.leakage,
                                                                      self.A_data, self.A_indices,
                                                self.A_indptr, self.A_shape, self.B_data, self.B_indices, self.B_indptr,
                                                self.B_shape, self.C, self.W, self.feature_size, self.squarenodes,
                                                self.input_pass, self.output_bias)
...
class NGRC():
    def __init__(self, input_size=3, k_time_delay=2, delay_s_steps=1, regularization=0.,polynomial_degree=2, train_noise=0., noise_seed=10):
        self.input_size = input_size
        self.k_time_delay = k_time_delay
        self.delay_s_steps = delay_s_steps
        self.regularization = regularization
        self.train_noise = train_noise
        self.rng = np.random.default_rng(noise_seed)
        self.polynomial_degree = polynomial_degree
        self.feature_size = 0
    def get_features(self,data,T,train=False): #this assumes p=2
        if train:
            train_data = data + self.rng.normal(size = data.shape)*self.train_noise
        else:
            train_data = data
        warm_up_len=self.delay_s_steps*self.k_time_delay
        linear_feature_size = self.input_size * self.k_time_delay
        linear_features = np.zeros((T,linear_feature_size))
        for k in range(self.k_time_delay):
            linear_features[:,k*self.input_size:(k+1)*self.input_size] = train_data[warm_up_len - (self.k_time_delay-1 - k) * self.delay_s_steps : warm_up_len - (self.k_time_delay-1 - k) * self.delay_s_steps + T] #this line assume s=1 for now
        index = np.triu_indices(linear_feature_size)
        non_linear_features = linear_features[:,index[0]] * linear_features[:,index[1]]
        features = np.hstack((linear_features,non_linear_features,np.ones((T,1))))
        self.feature_size = features.shape[1]
        return features
    def get_train_mats(self,data,T,return_error=False):
        warm_up_len = self.delay_s_steps * self.k_time_delay
        #output = data[warm_up_len - (self.k_time_delay-1)*self.delay_s_steps + 1:warm_up_len - (self.k_time_delay-1)*self.delay_s_steps + T + 1] - data[warm_up_len - (self.k_time_delay-1)*self.delay_s_steps :warm_up_len - (self.k_time_delay-1)*self.delay_s_steps + T]
        output = data[warm_up_len + 1:warm_up_len + T + 1] - data[warm_up_len : warm_up_len + T]
        features = self.get_features(data,T)
        info_mat = features.T @ features / T
        target_mat = features.T @ output / T
        if return_error:
            return info_mat,target_mat,features, output
    ...
    def train_from_mats(self,info_mat, target_mat,return_error=False,features=None,output=None):
        self.W = np.linalg.solve(info_mat + np.diag(
            np.ones(info_mat.shape[1]) * self.regularization), target_mat)
        if return_error:
            train_preds = features @ self.W
            train_error = mean_numba_axis1((train_preds - output) ** 2.0)
            return train_preds, train_error
    def train(self, data, T,return_error=False):
        if return_error:
            info_mat, target_mat, features, output = self.get_train_mats(data,T,return_error=True)
            train_preds,train_error = self.train_from_mats(info_mat,target_mat, features=features, output=output,return_error=True)
            return train_preds, train_error, output
        else:
            info_mat, target_mat = self.get_train_mats(data, T)
            self.train_from_mats(info_mat,target_mat)

    def predict(self, data, T):  # assume we are predicting warm_up_len+1 to warm_up_len+T
        warm_up_len = self.delay_s_steps * self.k_time_delay
        if data.shape[0] < warm_up_len + 1:
            raise ValueError('Not enough data to make prediction')
        truth=data.copy()
        pred = np.zeros((T, self.input_size))
        pred[:warm_up_len+1]=data[:warm_up_len+1]
        linear_feature_size = self.input_size * self.k_time_delay
        for i in range(T):  # open loop for all
            linear_features = np.zeros((1, linear_feature_size))
            for k in range(self.k_time_delay):
                linear_features[:, k * self.input_size:(k + 1) * self.input_size] = data[warm_up_len - (self.k_time_delay - 1 - k) * self.delay_s_steps +i : warm_up_len - (self.k_time_delay - 1 - k) * self.delay_s_steps + 1+i]  # this line assume s=1 for now
            index = np.triu_indices(linear_feature_size)
            non_linear_features = linear_features[:, index[0]] * linear_features[:, index[1]]
            features = np.hstack((linear_features, non_linear_features, np.ones((1, 1))))
            pred[i] = features @ self.W + data[warm_up_len + i]
            data[warm_up_len + i + 1] = pred[i]
        return pred, truth[warm_up_len+1:warm_up_len+T+1]
...
class Kernel_NGRC():
    def __init__(self, input_size=3, k_time_delay=2, delay_s_steps=1, regularization=0.,polynomial_degree=2, train_noise=0., noise_seed=10, sigma=1.0):
        self.input_size = input_size
        self.k_time_delay = k_time_delay
        self.delay_s_steps = delay_s_steps
        self.regularization = regularization
    ...
    def train(self,data,T,return_error=False): #this assumes p=2
        ...
        for k in range(self.k_time_delay):
            linear_features[:, k * self.input_size:(k + 1) * self.input_size] = train_data[warm_up_len - (self.k_time_delay - 1 - k) * self.delay_s_steps: warm_up_len - (self.k_time_delay - 1 - k) * self.delay_s_steps + T]  # this line assume s=1 for now
        K = RBFK(linear_features,linear_features,sigma=self.sigma)
        self.linear_features=linear_features
        self.K_inv = np.linalg.inv(K+self.regularization*np.identity(T))
        self.output = data[warm_up_len + 1:warm_up_len + T + 1] - data[warm_up_len : warm_up_len + T]
        if return_error:
    ...
    def predict(self, data, T):  # assume we are predicting warm_up_len+1 to warm_up_len+T
        ...
        for i in range(T):  # open loop for all
            ...
            for k in range(self.k_time_delay):
                linear_features[:, k * self.input_size:(k + 1) * self.input_size] = data[warm_up_len - (self.k_time_delay - 1 - k) * self.delay_s_steps +i : warm_up_len - (self.k_time_delay - 1 - k) * self.delay_s_steps + 1+i]  # this line assume s=1 for now
            dx = self.output.T @ self.K_inv @ RBFK(self.linear_features, linear_features, sigma=self.sigma)
            pred[i] = dx.T + data[warm_up_len + i]
            data[warm_up_len + i + 1] = pred[i]
        print(np.amax((pred-truth[warm_up_len+1:warm_up_len+T+1])**2))
        return pred, truth[warm_up_len+1:warm_up_len+T+1]
...
class Kernel_Reservoir():
    ...
    def train(self, data, T, discard_len=0, train=False, return_error=False):
        if train:
            train_data = data + self.rng.normal(size=data.shape) * self.train_noise
        else:
            train_data = data
        ...
        K = RBFK(features,features,sigma=self.sigma)
        self.K_inv = np.linalg.inv(K+self.regularization*np.identity(T))
...
spec = [
    ('bias_feature', float64[:]),
    ('W', float64[:,:]),
    ('H', float64[:,:]),
    ('state', float64[:]),
    ('model_bias', float64[:]),
    ('regularization', float64),
    ('reservoir_train_noise', float64),
    ('model_difference', boolean),
    ('prior',boolean)
]
#@jitclass(spec)
